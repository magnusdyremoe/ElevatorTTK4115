 ...
 * C source code generated on : Tue Oct 01 16:30:08 2019
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* S-Function Block: heli_q8/Heli 3D/HIL Read Encoder Timebase (hil_read_encoder_timebase_block) */
    {
      ...
      if (result < 0) {
        ...
        rtb_Gain1 = heli_q8_DW.HILReadEncoderTimebase_Buffer[2];
      }
    }

    /* Sum: '<S2>/Add1' incorporates:
     *  Constant: '<S2>/e offset '
     *  Gain: '<S2>/Elevation: Count to rad'
     */
    heli_q8_B.Add1 = heli_q8_P.ElevationCounttorad_Gain * rtb_Gain1 +
      heli_q8_P.eoffset_Value;

    /* Sum: '<S2>/Add' incorporates:
     *  Constant: '<S2>/p offset'
     *  Gain: '<S2>/Pitch: Count to rad'
     */
    heli_q8_B.Add = heli_q8_P.PitchCounttorad_Gain *
      rtb_HILReadEncoderTimebase_o2 + heli_q8_P.poffset_Value;
    ...
    if (heli_q8_B.RateTransitionx > heli_q8_P.DeadZonex_End) {
      rtb_Gain1 = heli_q8_B.RateTransitionx - heli_q8_P.DeadZonex_End;
    } else if (heli_q8_B.RateTransitionx >= heli_q8_P.DeadZonex_Start) {
      rtb_Gain1 = 0.0;
    } else {
      rtb_Gain1 = heli_q8_B.RateTransitionx - heli_q8_P.DeadZonex_Start;
    }
     ...
     *  Gain: '<S3>/Gain: x'
     */
    heli_q8_B.Joystick_gain_x = heli_q8_P.Gainx_Gain * rtb_Gain1 *
    ...
    if (heli_q8_B.RateTransitiony > heli_q8_P.DeadZoney_End) {
      rtb_Gain1 = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_End;
    } else if (heli_q8_B.RateTransitiony >= heli_q8_P.DeadZoney_Start) {
      rtb_Gain1 = 0.0;
    } else {
      rtb_Gain1 = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_Start;
    }
     ...
     *  Gain: '<S3>/Gain: y'
     */
    heli_q8_B.Joystick_gain_y = heli_q8_P.Gainy_Gain * rtb_Gain1 *
      heli_q8_P.Joystick_gain_y;

    /* Gain: '<Root>/Gain1' */
    rtb_Gain1 = heli_q8_P.Gain1_Gain * heli_q8_B.Joystick_gain_y;

    /* Gain: '<S4>/F' incorporates:
     *  SignalConversion: '<S4>/TmpSignal ConversionAtFInport1'
     */
    heli_q8_B.F[0] = 0.0;
    heli_q8_B.F[0] += heli_q8_P.F[0] * heli_q8_B.Gain;
    heli_q8_B.F[0] += heli_q8_P.F[2] * rtb_Gain1;
    heli_q8_B.F[1] = 0.0;
    heli_q8_B.F[1] += heli_q8_P.F[1] * heli_q8_B.Gain;
    heli_q8_B.F[1] += heli_q8_P.F[3] * rtb_Gain1;
  }
   ...
   */
  for (i = 0; i < 2; i++) {
    rtb_Sum[i] = heli_q8_B.F[i] - ((heli_q8_P.K[i + 2] *
      heli_q8_B.PitchTransferFcn + heli_q8_P.K[i] * heli_q8_B.Add) +
      heli_q8_P.K[i + 4] * heli_q8_B.ElevationTransferFcn);
  }

  /* End of Sum: '<S4>/Sum' */
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    /* Constant: '<S4>/V_s_0 ' */
    heli_q8_B.V_s_0 = heli_q8_P.V_s_0_Value;
  }

  /* Sum: '<S4>/Sum1' */
  heli_q8_B.Sum1 = rtb_Sum[0] + heli_q8_B.V_s_0;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (heli_q8_B.Sum1 - rtb_Sum[1]) * heli_q8_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (heli_q8_B.Sum1 + rtb_Sum[1]) * heli_q8_P.Backgain_Gain;
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (4158817536U);
  heli_q8_M->Sizes.checksums[1] = (2854855969U);
  heli_q8_M->Sizes.checksums[2] = (3649787160U);
  heli_q8_M->Sizes.checksums[3] = (3463680237U);
  ...
  heli_q8_M->Sizes.numBlocks = (49);   /* Number of blocks */
  heli_q8_M->Sizes.numBlockIO = (18);  /* Number of block outputs */
  heli_q8_M->Sizes.numBlockPrms = (150);/* Sum of parameter "widths" */
  return heli_q8_M;
}
